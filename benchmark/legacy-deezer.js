...
class DeezerLegacy {
	...
	async #ensureSession() {
		if (this.#currentSessionTimestamp + DeezerLegacy.#SESSION_EXPIRE > Date.now()) return;
	...
	async search(query, type) {
		...
		type = DeezerLegacy.#ENTITY_TYPES.find(e => e === type?.toLowerCase?.()) ?? "track";
	...
	async get(idOrURL, type) {
		...
		if (type) {
			...
			type = DeezerLegacy.#ENTITY_TYPES.find(e => idOrURL.toLowerCase().includes(e)) ?? "track";
	...
	async getAndDecryptTrack(track, flac = false) {
		...
		if (!Number(track.FILESIZE) && track.FALLBACK) track = track.FALLBACK;
		...
		const buffer = await this.#request(url, { buffer: true }),
			...
			blowfishKey = blowfish.key(
				Array(16)
					.fill(0)
					.reduce((acc, _, i) => acc + String.fromCharCode(md5.charCodeAt(i) ^ md5.charCodeAt(i + 16) ^ DeezerLegacy.#CBC_KEY.charCodeAt(i)), "")
			),
		...
		while (position < buffer.length) {
			...
			chunk = i % 3 || chunkSize < 2048 ? chunk.toString("binary") : blowfish.cbc(blowfishKey, Buffer.from([0, 1, 2, 3, 4, 5, 6, 7]), chunk, true).toString("binary");
